...
from torch.utils.data import Subset  # add at the top with other imports
...
def data_init(args):
    if args.dataset_name == 'imagenet':
        ...
        args.x_size = (3, 32, 32)
        args.y_size = (3, 32, 32)
    elif args.dataset_name == 'cifar10_full_test':
        valid_set = datasets.cifar10(args.dataset_root, mode="full_test")
        args.x_size = (3, 32, 32)
        args.y_size = (3, 32, 32)
    elif args.dataset_name == 'cifar10_unseen_test':
        valid_set = datasets.cifar10(args.dataset_root, mode="unseen_test")
        args.x_size = (3, 32, 32)
        args.y_size = (3, 32, 32)
    else:
        raise NotImplementedError

    # NEW: limit number of images if requested
    if getattr(args, "max_images", -1) is not None and args.max_images > 0:
        n = min(args.max_images, len(valid_set))
        print(f"[INFO] Using only first {n} images out of {len(valid_set)}")
        valid_set = Subset(valid_set, list(range(n)))
...
def model_init(args):
    ...
    elif args.dataset_name in ('cifar10', 'cifar10_full_test', 'cifar10_unseen_test'):
...
def attacker_init(args):
    ...
    attack_dataset = "cifar10" if dataset in ("cifar10_full_test", "cifar10_unseen_test") else dataset
    ...
    elif args.attack_method == 'my_attack_mcg':
        attacker = attacks.MyAttack(
            dataset_name=attack_dataset,
            max_query=max_query,
            targeted=targeted,
            class_num=class_num,
    ...
    elif args.attack_method == 'my_attack_plain':
        attacker = attacks.MyAttack(
            dataset_name=attack_dataset,
            max_query=max_query,
            targeted=targeted,
    ...
    elif args.attack_method == 'hybrid_ga':   # NEW
        attacker = attacks.HybridGA(
            dataset_name=attack_dataset,
            max_query=max_query,
            targeted=targeted,
            class_num=class_num,
