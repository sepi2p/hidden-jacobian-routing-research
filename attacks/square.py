...
class SquareAttack(BaseAttack):
    ...
    def attack(self, loss_func, x, y, init=None, buffer=None, **kwargs):
        ...
        if init is None:
            ...
            init = np.random.choice([-self.linf, self.linf],
                                    size=[x.shape[0], 3, 1, x.shape[-1]])

        x_best = torch.clamp(x + init, 0., 1.)
        output = loss_func(x_best, y, self.targeted)
        margin_min, logits_best, loss_min = \
            output['margin'], output['logits'], output['loss']
        query_cnt = 0
        success = False

        # ------------------------------
        # Track initial MCG score (prob-based)
        # ------------------------------
        # logits_best is probability vector from margin_loss_interface
        label = int(y) if not torch.is_tensor(y) else int(y.item())
        p_label = logits_best[0, label].item()
        if not self.targeted:
            initial_prob_score = 1.0 - p_label  # untargeted: 1 - p(true)
        else:
            initial_prob_score = p_label        # targeted:     p(target)
        # ------------------------------
        ...
        for i_iter in range(self.max_query - 1):
            ...
            x_best_curr_window = \
                x_best[:, :, center_h:center_h + s, center_w:center_w + s]

            # Avoid overlapping window that doesn't change anything
            while torch.sum(torch.abs(torch.clamp(
                    x_curr_window + perturbation[:, :,
                    center_h:center_h + s, center_w:center_w + s],
                    0., 1.)
                    - x_best_curr_window) < 1e-7) == self.c * s * s:
                perturbation[:, :,
                center_h:center_h + s,
                center_w:center_w + s] = torch.tensor(
                    np.random.choice([-self.linf, self.linf],
                                     size=[self.c, 1, 1])).cuda()
        ...
        # ------------------------------
        # Print MCG score only if failed
        # ------------------------------
        if not success:
            print(f"[INFO] Attack failed. Initial MCG score: {initial_prob_score:.6f}")
        # ------------------------------
