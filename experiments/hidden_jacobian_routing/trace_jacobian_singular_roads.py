...
from experiments.hidden_jacobian_routing.common import load_model as load_feature_model, margin, project_linf  # noqa: E402
...
def logits_stats(model, x: torch.Tensor, y: torch.Tensor):
    with torch.no_grad():
        logits = model(x)
...
def jvp_strengths_randomized(model, x: torch.Tensor, n_dirs: int, seed: int) -> np.ndarray:
    vals = []
    g = torch.Generator(device=x.device).manual_seed(seed)
...
def main():
    ...
    p.add_argument("--output-dir", default="analysis_outputs/hidden_jacobian_routing/singular_road_tracing_resnet50_quick")
