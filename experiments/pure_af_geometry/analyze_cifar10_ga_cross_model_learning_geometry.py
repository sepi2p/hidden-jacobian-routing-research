#!/usr/bin/env python3
"""CIFAR-10 GA success-flow geometry across independently trained models.

The experiment asks whether random-noise-to-pure GA trajectories reveal a
reproducible representation-space success-flow geometry across learned CIFAR-10
classifiers, not just one architecture.
"""
...
class CIFARFeatureWrapper(nn.Module):
    ...
    def _register_hooks(self) -> list[str]:
        ...
        if self.spec == "bbb_resnet50":
            for label, module_name in [("layer3", "1.layer3"), ("layer4", "1.layer4")]:
                if self._register(module_name, label):
                    labels.append(label)
        elif self.spec == "bbb_vgg19_bn":
            for label, module_name in [("final_conv", "1.features.51"), ("penultimate", "1.features")]:
                if self._register(module_name, label):
                    labels.append(label)
        elif self.spec == "bbb_densenet":
            if self._register("1.features", "final_conv"):
                labels.append("final_conv")
        elif self.spec == "bbb_inception_v3":
            # The local CIFAR Inception variant exposes many convs; use final conv fallback.
            pass
        if not labels:
            last_conv = None
...
def analyze(args, out_dir):
    ...
    clean_rows = pd.read_csv(out_dir / "clean_motion.csv")
    ...
    dim_rows, pred_rows, transfer_rows, sim_rows = [], [], [], []
    ...
    for (model, layer), group in seg.groupby(["model", "layer"]):
        ...
        for variant, vectors, local_group in [("raw", normalize_rows(x), group)]:
            ...
            basis_cache[(model, layer, variant)] = (mean, basis, feature_basis(vectors[train], max(KS)))
            pos = vectors[test]
            random_x = normalize_rows(rng.normal(size=(max(len(pos), 50), vectors.shape[1])))
            for comp, neg in [("success_vs_random", random_x), ("success_vs_clean", normalize_rows(clean) if len(clean) else np.empty((0, vectors.shape[1])))]:
                for row in compare(pos, neg, mean, basis):
                    row.update({"model": model, "layer": layer, "variant": variant, "comparison": comp})
                    pred_rows.append(row)
                acc = logistic_acc(pos, neg, mean, basis)
                pred_rows.append({"model": model, "layer": layer, "variant": variant, "comparison": comp, "k": "all", "logistic_accuracy": acc, "n_pos": len(pos), "n_neg": len(neg)})
        if len(g) == len(x):
            orth, keep = orthogonalize(x, g)
            og = group.iloc[np.flatnonzero(keep)].reset_index(drop=True)
            success = og["success"].to_numpy(dtype=int) == 1
            success_runs = sorted(og.loc[success, "run_id"].unique())
            if len(success_runs) >= 3:
                train_runs, test_runs = train_test_split(success_runs, test_size=0.35, random_state=args.seed)
                train = success & og["run_id"].isin(train_runs).to_numpy()
                test = success & og["run_id"].isin(test_runs).to_numpy()
                if train.sum() >= 10 and test.sum() >= 5:
                    stats = pca_stats(orth[train])
                    stats.update({"model": model, "layer": layer, "variant": "gradient_orthogonalized"})
                    dim_rows.append(stats)
                    mean, basis = fit_basis(orth[train], max(KS))
                    basis_cache[(model, layer, "gradient_orthogonalized")] = (mean, basis, feature_basis(orth[train], max(KS)))
                    pos = orth[test]
                    random_x = normalize_rows(rng.normal(size=(max(len(pos), 50), orth.shape[1])))
                    for comp, neg in [("success_vs_random", random_x), ("success_vs_clean", normalize_rows(clean) if len(clean) else np.empty((0, orth.shape[1])))]:
                        for row in compare(pos, neg, mean, basis):
                            row.update({"model": model, "layer": layer, "variant": "gradient_orthogonalized", "comparison": comp})
                            pred_rows.append(row)

    for (train_model, layer, variant), (mean, basis, fbasis) in basis_cache.items():
        for (test_model, test_layer), group in seg.groupby(["model", "layer"]):
            if test_layer != layer:
                continue
            x = normalize_rows(get_npz(vec_npz, "vectors", test_model, test_layer))
            if len(x) == 0 or x.shape[1] != mean.shape[1]:
                continue
            pos = x
            random_x = normalize_rows(rng.normal(size=(len(pos), x.shape[1])))
            for row in compare(pos, random_x, mean, basis):
                row.update({"train_model": train_model, "test_model": test_model, "layer": layer, "variant": variant, "comparison": "success_vs_random"})
                transfer_rows.append(row)
            acc = logistic_acc(pos, random_x, mean, basis)
            transfer_rows.append({"train_model": train_model, "test_model": test_model, "layer": layer, "variant": variant, "comparison": "success_vs_random", "k": "all", "logistic_accuracy": acc, "n_pos": len(pos), "n_neg": len(random_x)})

    for layer in sorted(seg["layer"].unique()):
        keys = [k for k in basis_cache if k[1] == layer and k[2] == "raw"]
        for i, ka in enumerate(keys):
            for kb in keys[i + 1:]:
                ba = basis_cache[ka][2]
                bb = basis_cache[kb][2]
                for k in [20, 50, 100]:
                    row = subspace_metrics(ba, bb, k)
                    if row:
                        row.update({"model_a": ka[0], "model_b": kb[0], "layer": layer, "variant": "raw"})
                        sim_rows.append(row)

    pd.DataFrame(dim_rows).to_csv(out_dir / "cifar_cross_model_geometry.csv", index=False)
    # No local multi-seed checkpoints were found; write same schema with available model identities.
    pd.DataFrame(dim_rows).to_csv(out_dir / "cifar_cross_seed_geometry.csv", index=False)
    pd.DataFrame(pred_rows).to_csv(out_dir / "cifar_predictiveness_metrics.csv", index=False)
    pd.DataFrame(transfer_rows).to_csv(out_dir / "cifar_basis_transfer_metrics.csv", index=False)
    pd.DataFrame(sim_rows).to_csv(out_dir / "cifar_geometry_similarity_matrix.csv", index=False)
    plot_summary(pd.DataFrame(dim_rows), pd.DataFrame(pred_rows), out_dir)
...
def plot_summary(dim_df, pred_df, out_dir):
    ...
    if len(dim_df):
        ...
        axes[0].set_title("CIFAR GA success-flow dimensionality")
    if len(pred_df) and "auroc" in pred_df:
        sub = pred_df[(pred_df["k"].astype(str) == "100") & (pred_df["comparison"] == "success_vs_random") & (pred_df["variant"] == "raw")]
        ...
        axes[1].set_title("Success-vs-random predictiveness")
    fig.savefig(out_dir / "cifar_geometry_summary.png", dpi=180)
    plt.close(fig)


def main():
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--output-dir", default="analysis_outputs/pure_af_geometry/cifar_ga_cross_model_learning_geometry")
