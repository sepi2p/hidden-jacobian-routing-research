#!/usr/bin/env python3
"""Expanded CIFAR optimizer transport trajectories using learned transport axes.

This extends the original PGD/Square trajectory logger with several
gradient-based and query/search-based optimizers. It records feature
displacements over time and projects those displacements onto the learned
transport axes used by the cross-optimizer analysis.
"""
...
def random_search_trajectory(wrapper, x: torch.Tensor, y: torch.Tensor, eps: float, steps: int, seed: int):
    gen = torch.Generator(device=x.device).manual_seed(seed)
    x0 = x.detach()
    x_adv = x0.clone()
    states = [x_adv.detach().clone()]
...
def main():
    ...
    p.add_argument("--output-dir", default="analysis_outputs/pure_af_geometry/cifar_expanded_optimizer_transport")
    p.add_argument("--dataset-root", default="/home/sepi/data/cifar10")
    p.add_argument("--models", default="bbb_resnet50,bbb_vgg19_bn,bbb_densenet,bbb_inception_v3")
    p.add_argument("--attacks", default="pgd,mi_fgsm,ni_fgsm,ti_fgsm,si_fgsm,square,nes,random_search,bandit")
    ...
    p.add_argument("--query-steps", type=int, default=40)
